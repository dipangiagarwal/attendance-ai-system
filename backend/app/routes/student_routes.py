...
from app.controllers.student_controller import ( create_student as create_student_controller, get_all_students, get_student_by_id, update_student as update_student_controller, delete_student as delete_student_controller)
...
async def add_student(
    ...
    photo: Optional[UploadFile] = File(None),  # ← Cloudinary upload
    ...
    return create_student_controller(db, student_data, photo=photo_file)
...
def patch_student(
    ...
    updated = update_student_controller(db, student_id, student)
    if not updated:
        raise HTTPException(status_code=404, detail="Student not found")
    return updated
...
def remove_student(
    ...
    deleted = delete_student_controller(db, student_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Student not found")
